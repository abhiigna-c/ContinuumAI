    ...
    <div className="p-8">
      <div className="max-w-2xl">
        ...
        <div className="bg-card rounded-lg shadow-sm border border-border/40 p-8 hover:shadow-md hover:border-border/60 transition-all duration-300">
          <h3 className="text-2xl font-semibold text-foreground mb-6">Describe Your Symptoms</h3>

          <textarea
            value={symptoms}
            onChange={(e) => setSymptoms(e.target.value)}
            placeholder="Describe what you're experiencing... (e.g., severe headache, dizziness, chest pain)"
            className="w-full h-32 px-4 py-3 border border-border/40 rounded-lg bg-input text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-primary focus:border-transparent resize-none transition"
          />

          <button
            onClick={handleCheck}
            disabled={!symptoms || loading}
            className="mt-6 px-8 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition disabled:opacity-50"
          >
            ...
            >
              <div className="flex gap-3 mb-3">
                <CheckCircle
                  ...
                  size={24}
                />
                <h4 className="text-lg font-semibold text-foreground">Risk Level: {result.riskLevel}</h4>
              </div>
